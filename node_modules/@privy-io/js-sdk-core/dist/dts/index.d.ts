...
declare class PrivyApiError extends Error {
    ...
    code: PrivyErrorCode;
    status: number;
    constructor({ error, code, status }: ServerErrorBody);
}
...
type MfaSubmitArgs = {
    mfaMethod: MfaMethod;
    mfaCode: string | PasskeyAuthenticatorVerifyResponse;
    relyingParty: string;
};
type MfaPromise = {
    current: {
...
declare class EmbeddedWalletApi {
    /**
     * @experimental
     * Sign a message with the user's signer.
     *
     * @param o input options
     * @param o.message the base64 encoded message to sign
     *
     * @returns an object containing the base64 encoded message signature
     */
    signWithUserSigner(opts: {
        message: string;
    }): Promise<{
        signature: string;
    }>;
...
declare class CustomProviderApi {
    ...
    /**
     * Links a custom JWT account to an existing logged-in user
     *
     * @param token The JWT from the non-Privy service
     */
    linkWithToken(token: string): Promise<{
        user: _privy_io_api_types.User;
...
declare class PasskeyApi {
     ...
     */
    generateRegistrationOptions(relyingParty?: string): Promise<_privy_io_api_types.ResponsePasskeyInitLink>;
    /**
     * Begin a login flow with a passkey.
     */
    generateAuthenticationOptions(relyingParty?: string): Promise<_privy_io_api_types.ResponsePasskeyInitAuthenticate>;
    /**
     * Begin a sign up flow with a passkey.
     */
    generateSignupOptions(relyingParty?: string): Promise<_privy_io_api_types.ResponsePasskeyInitRegister>;
    /**
...
declare class RecoveryICloudApi {
     ...
     */
    getICloudConfiguration(client_type: 'web' | 'expo-ios'): Promise<_privy_io_api_types.RecoveryConfigurationICloudResponse>;
}
...
declare class RecoveryOAuthApi {
     ...
     * @param returnedStateCode The state value initially set in the request by Privy to the authorization server
     */
    authorize(authorizationCode: string, returnedStateCode: string): Promise<_privy_io_api_types.OAuthAuthenticateRecoveryResponse>;
}
...
declare class RecoveryApi {
     ...
     */
    getRecoveryKeyMaterial(address: string, chain_type?: string): Promise<_privy_io_api_types.RecoveryKeyMaterialResponse>;
}
...
declare const PRIVY_REQUEST_EXPIRY_HEADER_NAME: "privy-request-expiry";
/**
...
interface GenerateAuthorizationSignatureInput {
    ...
    headers: {
        ...
        [PRIVY_REQUEST_EXPIRY_HEADER_NAME]?: string;
    };
}
 ...
 */
declare function rawSign(context: WalletApiActionContext, sign: SignWalletRequest, { wallet_id, ...request }: Input$1): Promise<RawSignResponse>;
